...
import { Library, Package, Trophy, Users, Settings, Gamepad2, Shield } from "lucide-react";
...
export default function Sidebar() {
  ...
  useEffect(() => {
    setAdminUnlocked(localStorage.getItem("sakura_admin_unlocked") === "true");
    const check = () => setAdminUnlocked(localStorage.getItem("sakura_admin_unlocked") === "true");
      ...
      {adminUnlocked && (
        <NavLink
          ...
          className={({ isActive }) =>
            `w-10 h-10 rounded-xl flex items-center justify-center transition-all group relative ${
              isActive
                ? "bg-gradient-to-br from-purple-500/20 to-pink-500/20 text-purple-400 shadow-glow-sm"
                : "text-sakura-muted hover:text-purple-400 hover:bg-purple-500/10"
            }`
          }
        >
          {({ isActive }) => (
